...
if [ $load == "yes" ]; then
  ...
  #ifconfig sw0 192.168.178.201 up
  #sleep 1
  ...
  if [ $load0 == "yes" ]; then
    ...
    ifconfig sw0p0 up
    #(sleep 2; ethtool -s sw0p0 speed 100 autoneg off)
  fi
  if [ $load1 == "yes" ]; then
    echo "------- ETHA 1 --------"
    sysctl -w net.ipv6.conf.sw0p1.disable_ipv6=1
    ifconfig sw0p1 up
    #(sleep 2; ethtool -s sw0p1 speed 100 autoneg off)
  fi
  if [ $load2 == "yes" ]; then
    echo "------- ETHA 2 --------"
    sysctl -w net.ipv6.conf.sw0p2.disable_ipv6=1
    ifconfig sw0p2 up
    #(sleep 2; ethtool -s sw0p2 speed 100 autoneg off)
  fi
else
