...
extern const char * const ClockModeStr(enum ClockMode clockmode)
{
	switch (clockmode)
	{
		case ClockMode_Unknown:
		    return "UNKNOWN";
		case ClockMode_OC:
...
extern const char * const ClockTypeStr(enum clock_type clockType)
{
    switch (clockType)
    {
...
int generate_clock_identity(int PortNumber, struct ClockIdentity *ci, const char *name)
{
	unsigned char mac[6];
	if (sk_interface_macaddr(PortNumber, name, mac, sizeof(mac)))
		return -1;
	ci->id[0] = mac[0];
	ci->id[1] = mac[1];
	ci->id[2] = mac[2];
