...
struct clock 
{
    ...
    struct timePropertiesDS tds;                //< @brief Source, Offset, Mode
    ...
    bool                grand_master_capable;   ///< @brief We can operate as Grand Master Clock when TRUE (802.1AS only)
    bool                tcCapable;              ///< @brief We can operate as Transparent clock when TRUE (802.1AS only)
	int utc_timescale;
	int leap_set;
	int kernel_leap;
...
#ifdef OPT_LOG_LEVEL_FILE
/**
    @brief  Log Level control file
*/
static int      inotify_fd = -1;
static int      inotify_wd = -1;
#endif /* OPT_LOG_LEVEL_FILE */



/**
    @brief  Our Local Clock
*/
...
extern struct config * QueryConfig(void);



#ifdef OPT_LOG_LEVEL_FILE
/**
    @brief  Add inotify on Log Level control file

    External update of this file will result in change to log level
*/
static int init_inotify_fd()
{
    char    Command[256];
    FILE  * File = NULL;

    /*
        Create temp directory for Log Level control, if it does not exist
    */
    sprintf(Command, "mkdir -p \"%s\"", LOGLEVEL_CFG_DIR);
    system(Command);


    /*
        Create/update file with command-line Log Level
    */
    sprintf(Command, "%s%s", LOGLEVEL_CFG_DIR, LOGLEVEL_CFG_FILE);
    if ((File = fopen(Command, "w")) == NULL)
    {
        pr_err("Failed to create LogLevel File '%s' - %d (%s)", Command, errno, strerror(errno));
        return -1;
    }
    fprintf(File, "%d\n", print_query_level());
    fclose(File);
    ...
    /*
        Add an inotify on this file
    */
    inotify_fd = inotify_init();
    //pr_info("fd: %i", inotify_fd);
    if (inotify_fd < 0)
    {
    ...
    inotify_wd = inotify_add_watch(inotify_fd, Command, IN_CLOSE_WRITE);
    //pr_info("wd: %i", inotify_wd);
    if (inotify_wd < 0)
    {
        pr_info("inotify wd setup failed for '%s' - %d (%s)", Command, errno, strerror(errno));
        return -1;
    }
...
extern enum ClockMode Query_Clock_Mode(struct clock * c)
{
    return c->tds.ClockMode;
}


 extern tmv_t port_get_segress(struct clock *c, int portNum)
{
    return c->sync_eg[portNum];
}
extern tmv_t port_get_singress(struct clock *c)
{
    return c->sync_in;
}
void port_set_segress(struct clock *c, tmv_t val, int portNum)
{
    //pr_debug("TRANSPARENT: egress= %"PRId64, val);
    c->sync_eg[portNum] = val;
}
void port_set_singress(struct clock *c, tmv_t val)
{
    //pr_debug("TRANSPARENT: ingress= %"PRId64, val);
    c->sync_in = val;
}
			   ...
			   enum servo_type servo)
{
    ...
    c->tcCapable = dds->tcCapable;
//pr_info("setting to unknown");
    c->tds.ClockMode = ClockMode_Unknown;
pr_info("clockmode locn: %p", &(c->tds.ClockMode));
    ...
    */
    ...
    /*
        An extra FD for the inotify instance
    */
    init_inotify_fd();
#endif /* OPT_LOG_LEVEL_FILE */
...
int clock_poll(struct clock *c)
{
	...
	for (i = 0; i < c->nports; i++) {
		...
		for (j = err = 0; j < N_POLLFD && !err; j++) {
            ...
            {
				event = port_event(c->port, c->nports, c->port[i], j);
...
#ifdef OPT_LOG_LEVEL_FILE
    /*
        Poll for any changes in /var/gptp/LogLevel.cfg
    */
    struct pollfd pfd = {inotify_fd, POLLIN, 0};
    int ret = poll(&pfd, 1, 0);  
    if (ret < 0) 
    {
        pr_err("[LogLevel] poll failed: %s\n", strerror(errno));
    } 
    else if (ret == 0) 
    {
    ...
    else if (pfd.revents != 0)
    {
        ...
        int nbytes = read(inotify_fd, &i_event, sizeof(i_event));
        if (nbytes < 0)
        {
            pr_err("[LogLevel] Nothing read from inotify: %m");
        }
        else
        {
            FILE * fp = fopen(LOGLEVEL_CFG_DIR LOGLEVEL_CFG_FILE, "r");
            if (fp == NULL) 
            {
                pr_err("[LogLevel] Couldn't open '%s' for reading", 
                        LOGLEVEL_CFG_DIR LOGLEVEL_CFG_FILE);
            }
            else
            {
                if (fscanf(fp, "%u", &level) != 1)
                {
                    pr_err("[LogLevel] Couldn't read log level from '%s'", 
                            LOGLEVEL_CFG_DIR LOGLEVEL_CFG_FILE);
                }
                else if ( (level < PRINT_LEVEL_MIN) || (level > PRINT_LEVEL_MAX) )
                {
                    pr_err("[LogLevel] Couldn't use log level %d from '%s' - outside valid limits (%u to %u)", 
                            level,
                            LOGLEVEL_CFG_DIR LOGLEVEL_CFG_FILE,
                ...
                {
                    //printf("+++ Log level changed from %d to %d\n", print_query_level(), level);
                    pr_notice("[LogLevel] Log level changed from %d to %d", print_query_level(), level);
    ...
    /*
        Can only be Ordinary Clock with a single port
        When multiple ports, can be Transparent Clock or Grand Master Clock (Boundary Clock)
    */
    if (c->nports == 1)
    {
        c->tds.ClockMode = ClockMode_OC;
    }
    
    else if (c->tcCapable == 1)
    {
        int l;
        int MasterPorts =   0;
        int SlavePorts =    0;
        int UncalPorts =    0;
        static int NoPrintsGM = 0;
        static int NoPrintsTC = 0;
        for (l = 0; l < c->nports; l++)
        {   
            switch (Port_QueryState(c->port[l]))
            {
                case PS_SLAVE:
                    SlavePorts++;   
                    break;
        ...
        //pr_info("slaveports: %i masterports: %i uncalports: %i total ports: %i", SlavePorts, MasterPorts, UncalPorts, c->nports);
        if (MasterPorts == c->nports)
        {
            c->tds.ClockMode = ClockMode_GM;
            if(NoPrintsGM == 0)
            {   
                NoPrintsTC = 0;         
                pr_info("ENTERING GRANDMASTER MODE");
                NoPrintsGM++;
            }
        }
        else if ((MasterPorts + UncalPorts) == c->nports)
        {
            c->tds.ClockMode = ClockMode_GM;
            if(NoPrintsGM == 0)
            {   
        ...
        else 
        {
            ...
            {   
                NoPrintsGM = 0;         
                pr_info("ENTERING TRANSPARENT MODE");
                NoPrintsTC++;
            }
        }
    }
				   ...
				   Integer64 correction2)
{
	 ...
	 */
    /*pr_debug("Offset calc: %llu - %llu - %i - %llu - %llu", tmv_to_nanoseconds(ingress),tmv_to_nanoseconds(origin),
                                                   tmv_to_nanoseconds(c->path_delay), tmv_to_nanoseconds(c->c1),
	...
	if (c->stats.max_count > 1) {
		...
		pr_info("clock_sync master offset %10" PRId64 " s%d freq %+7.0f "
			"path delay %9" PRId64,
	...
	case SERVO_UNLOCKED:
		break;
	case SERVO_JUMP:
        pr_debug("****SERVO JUMP in SWITCH STATEMENT****");
		...
		break;
	case SERVO_LOCKED:
        pr_debug("****SERVO LOCKED in SWITCH STATEMENT****");
		clockadj_set_freq(c->clkid, -adj);
		if (c->clkid == CLOCK_REALTIME)
			sysclk_set_sync();
		if (c->sanity_check)
...
{   
    enum ClockMode CM = c->tds.ClockMode;
	c->tds = tds;
    c->tds.ClockMode = CM;
}
