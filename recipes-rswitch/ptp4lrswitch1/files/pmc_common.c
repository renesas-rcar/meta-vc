		       ...
		       UInteger8 transport_specific, int zero_datalen)
{
	...
	if (transport_type != TRANS_UDS &&
	    generate_clock_identity(0, &pmc->port_identity.clockIdentity,
				    iface_name)) {
	...
	if (transport_open(0, pmc->transport, iface_name, &pmc->fdarray, TS_SOFTWARE)) 
    {
...
{
	transport_close(99, pmc->transport, &pmc->fdarray);
	transport_destroy(pmc->transport);
	free(pmc);
}
...
static int pmc_send(struct pmc *pmc, struct ptp_message *msg, int pdulen)
{
	...
	if (err) {
		pr_err("[pmc] msg_pre_send failed");
		return -1;
	}
	cnt = transport_send(0, pmc->transport, &pmc->fdarray, 0, msg, pdulen, &msg->hwts);
	if (cnt < 0) {
		pr_err("[pmc] failed to send message");
		return -1;
	}
	return 0;
}
...
struct ptp_message *pmc_recv(struct pmc *pmc)
{
	...
	msg->hwts.type = TS_SOFTWARE;
	cnt = transport_recv(99, pmc->transport, pmc_get_transport_fd(pmc), msg, sizeof(msg->data), &msg->hwts);
	if (cnt <= 0) {
		pr_err("[pmc] recv message failed");
		goto failed;
	}
	...
	if (err) {
		switch (err) {
		case -EBADMSG:
			pr_err("[pmc] bad message received");
			break;
		case -ETIME:
			pr_err("[pmc] received %s without timestamp",
					msg_type_string(msg_type(msg)));
			break;
		case -EPROTO:
			pr_debug("[pmc] ignoring message");
		}
		goto failed;
	}
