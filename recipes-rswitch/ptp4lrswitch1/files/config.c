					       ...
					       struct config *cfg)
{
    ...
    else if (strcmp(option, "gmCapable") == 0) 
    {
        // When true, allow operation as a 802.1AS Grand Master Clock
        r = get_ranged_int(value, &val, 0, 1);
        if (r != PARSED_OK)
        {
            return r;
    ...
    else if (strcmp(option, "tcCapable") == 0) 
    {
        // When true and started with multiple ports, allows operation as a 802.1AS Transparent Switch
        r = get_ranged_int(value, &val, 0, 1);
        if (r != PARSED_OK)
        {
            return r;
        }
        cfg->dds.tcCapable = val;
    }
    /* 
        PresetMode={GM|TC} - Force mode of operation at startup (for fast startups)
        DisableAnnounce
    */
...
int config_create_interface(char *name, struct config *cfg)
{
	...
	sk_get_ts_info(99, name, &iface->ts_info);
