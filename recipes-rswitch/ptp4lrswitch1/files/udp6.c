...
static int udp6_close(int PortNumber, struct transport *t, struct fdarray *fda)
{
	close(fda->fd[0]);
	close(fda->fd[1]);
	return 0;
...
static int udp6_open(int PortNumber, struct transport *t, const char *name, struct fdarray *fda,
		    enum timestamp_type ts_type)
{
	...
	udp6->mac_len = 0;
	if (sk_interface_macaddr(PortNumber, name, udp6->mac, MAC_LEN) == 0)
		udp6->mac_len = MAC_LEN;

	udp6->ip_len = sk_interface_addr(PortNumber, name, AF_INET6, udp6->ip, sizeof(udp6->ip));
	if (udp6->ip_len == -1)
		udp6->ip_len = 0;
	...
	if (sk_timestamping_init(PortNumber, efd, name, ts_type, TRANS_UDP_IPV6))
...
static int udp6_recv(int PortNumber, struct transport *t, int fd, void *buf, int buflen, struct hw_timestamp *hwts)
{
	return sk_receive(PortNumber, fd, buf, buflen, hwts, 0);
}


static int udp6_send(int PortNumber, struct transport *t, struct fdarray *fda, int event, int peer,
...
static int udp6_physical_addr(int PortNumber, struct transport *t, uint8_t *addr)
{
	struct udp6 *udp6 = container_of(t, struct udp6, t);
	if (udp6->mac_len)
...
static int udp6_protocol_addr(int PortNumber, struct transport *t, uint8_t *addr)
{
	struct udp6 *udp6 = container_of(t, struct udp6, t);
	if (udp6->ip_len)
