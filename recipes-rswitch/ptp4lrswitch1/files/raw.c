...
static int raw_configure(int PortNumber, int fd, int event, int index,
			 unsigned char *addr1, unsigned char *addr2, int enable)
{
	...
	if (setsockopt(fd, SOL_SOCKET, SO_ATTACH_FILTER, &prg, sizeof(prg))) {
		pr_err("PortDev[%u] setsockopt SO_ATTACH_FILTER failed: %m", PortNumber);
		return -1;
	}
	...
	if (err1)
		pr_warning("PortDev[%u] setsockopt PACKET_MR_MULTICAST failed: %m", PortNumber);
	...
	pr_warning("PortDev[%u] setsockopt PACKET_MR_ALLMULTI failed: %m", PortNumber);
	...
	pr_warning("PortDev[%u] setsockopt PACKET_MR_PROMISC failed: %m", PortNumber);

	pr_err("PortDev[%u] all socket options failed", PortNumber);
	return -1;
}

static int raw_close(int PortNumber, struct transport *t, struct fdarray *fda)
{
	close(fda->fd[0]);
	close(fda->fd[1]);
	return 0;
...
static int open_socket(int PortNumber, const char *name, int event)
{
	...
	if (bind(fd, (struct sockaddr *) &addr, sizeof(addr))) {
		pr_err("PortDev[%u] bind failed: %m", PortNumber);
		goto no_option;
	}
	if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, name, strlen(name))) {
		pr_err("PortDev[%u] setsockopt SO_BINDTODEVICE failed: %m", PortNumber);
		goto no_option;
	}
	if (raw_configure(PortNumber, fd, event, index, ptp_dst_mac, p2p_dst_mac, 1))
		goto no_option;
...
static int raw_open(int PortNumber, struct transport *t, const char *name,
		    struct fdarray *fda, enum timestamp_type ts_type)
{
	...
	if (sk_interface_macaddr(PortNumber, name, raw->ptp_addr.src, MAC_LEN))
		goto no_mac;

	memcpy(raw->p2p_addr.src, raw->ptp_addr.src, MAC_LEN);

	efd = open_socket(PortNumber, name, 1);
	if (efd < 0)
		goto no_event;

	gfd = open_socket(PortNumber, name, 0);
	if (gfd < 0)
		goto no_general;

	if (sk_timestamping_init(PortNumber, efd, name, ts_type, TRANS_IEEE_802_3))
...
static int raw_recv(int PortNumber, struct transport *t, int fd, void *buf, int buflen, struct hw_timestamp *hwts)
{
	int cnt, hlen;
	...
	cnt = sk_receive(PortNumber, fd, ptr, buflen, hwts, 0);

	if (raw->vlan) {
		if (ETH_P_1588 == ntohs(hdr->type)) {
			pr_notice("PortDev[%u] raw: disabling VLAN mode", PortNumber);
			raw->vlan = 0;
		}
	} else {
		if (ETH_P_8021Q == ntohs(hdr->type)) {
			pr_notice("PortDev[%u] raw: switching to VLAN mode", PortNumber);
			raw->vlan = 1;
		}
	}
	if (cnt >= hlen)  {
...
static int raw_send(int PortNumber, struct transport *t, struct fdarray *fda, int event, int peer,
		    void *buf, int len, struct hw_timestamp *hwts)
{
	...
	if (cnt < 1) 
    {
        // 9 - Bad File Descritpor occurs when the port is inactive (no link)
        if (errno == EBADF)
        {
            pr_warning("PortDev[%u] send failed: %d %m (the port may be inactive)", PortNumber, errno);
        }
        else
        {
            pr_err("PortDev[%u] send failed: %d %m", PortNumber, errno);
        }
		return cnt;
	}
	/*
	 ...
	 */
	return event == TRANS_EVENT ? sk_receive(PortNumber, fd, pkt, len, hwts, MSG_ERRQUEUE) : cnt;
}
...
static int raw_physical_addr(int PortNumber, struct transport *t, uint8_t *addr)
{
	struct raw *raw = container_of(t, struct raw, t);
...
static int raw_protocol_addr(int PortNumber, struct transport *t, uint8_t *addr)
{
	struct raw *raw = container_of(t, struct raw, t);
	memcpy(addr, raw->ptp_addr.src, MAC_LEN);
	return MAC_LEN;
