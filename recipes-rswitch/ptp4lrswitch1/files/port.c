...
extern enum port_state Port_QueryState(struct port * Port)
{
    return Port->state;
}
...
/**
    ...
    @return
*/
static void timestamp_to_Timestamp(struct timestamp *src, struct Timestamp *dst)
{
    dst->seconds_lsb =src->sec;
    dst->seconds_msb = 0;
...
/**
    @brief  Calculate FOLLOW-UP Correction Time

    @param  ingress
    @param  egress
    @param  peer_delay          Peer delay for this port
    @param  Incoming_correction Correction value from ingress FOLLOW-UP message
*/
static int64_t calc_correction(tmv_t ingress, tmv_t egress, tmv_t peer_delay, Integer64 Incoming_correction)
{
    tmv_t     tvm_Incoming_correction = 0;
    tmv_t     correction = 0;
    Integer64 final_correction;

    tvm_Incoming_correction = correction_to_tmv(Incoming_correction);
    ...
    correction = egress - ingress;

    correction += peer_delay;

    pr_debug("out-t1(%" PRIu64 ") in-t2(%" PRId64 ") Correction(%" PRId64 ") In(%" PRId64 ") Out(%" PRId64 "). Pdelay(%" PRId64 ")", 
			egress, ingress, correction, tvm_Incoming_correction, correction + tvm_Incoming_correction, peer_delay);

    correction += tvm_Incoming_correction;
        
    if (correction < 0)
    {
        correction *= -1;
    }
    final_correction = tmv_to_correction(correction);
...
static int add_foreign_master(struct port *p, struct ptp_message *m)
{
	...
	if (!fc) {
		pr_notice("PortDev[%u] new foreign master %s", portnum(p), pid2str(&m->header.sourcePortIdentity));

		fc = malloc(sizeof(*fc));
		if (!fc) {
			pr_err("PortDev[%u] low memory, failed to add foreign master", portnum(p));
			return 0;
		}
...
static int port_capable(struct port *p)
{
	...
	if (tmv_to_nanoseconds(p->peer_delay) >	p->neighborPropDelayThresh) {
		if (p->asCapable)
			pr_debug("PortDev[%u] peer_delay (%" PRId64 ") > neighborPropDelayThresh "
	...
	if (p->pdr_missing > ALLOWED_LOST_RESPONSES) {
		if (p->asCapable)
			pr_debug("PortDev[%u] missed %d peer delay resp, "
	...
	if (p->multiple_seq_pdr_count) {
		if (p->asCapable)
			pr_debug("PortDev[%u] multiple sequential peer delay resp, "
	...
	if (!p->peer_portid_valid) {
		if (p->asCapable)
			pr_debug("PortDev[%u] invalid peer port id, "
				"resetting asCapable", portnum(p));
		goto not_capable;
	}
	...
	if (!p->nrate.ratio_valid) {
		if (p->asCapable)
			pr_debug("PortDev[%u] invalid nrate, "
				"resetting asCapable", portnum(p));
		goto not_capable;
	}
...
capable:
	if (!p->asCapable)
		pr_debug("PortDev[%u] setting asCapable", portnum(p));
	p->asCapable = 1;
	return 1;
				       ...
				       struct ptp_message *msg, int error_id)
{
	if (port_management_error(p->portIdentity, ingress, msg, error_id))
		pr_err("PortDev[%u] management error failed", portnum(p));
}
					...
					struct ptp_message *req)
{
	...
	case CLOCK_DESCRIPTION:
        ...
        *cd->clockType = CLOCK_TYPE_ORDINARY;
        ...
        {
            switch (Query_Clock_Mode(ingress->clock))
            {
                case ClockMode_Unknown:         // Unknown/unsupported
                    break;
                case ClockMode_BC:              // Boundary Clock
                    *cd->clockType = CLOCK_TYPE_BOUNDARY;
                    break;
                case ClockMode_OC:              // Ordinary Clock
                    *cd->clockType = CLOCK_TYPE_ORDINARY;
                    break;
                case ClockMode_TC:              // Transparent Clock
                    *cd->clockType = CLOCK_TYPE_TRANSPARENT;
                    break;
                case ClockMode_GM:              // Grand Master Clock
                    *cd->clockType = CLOCK_TYPE_TRANSPARENT;
                    break;
            }
        } 
		...
		u16 = transport_physical_addr(ingress->portIdentity.portNumber, target->trp,
		...
		u16 = transport_protocol_addr(ingress->portIdentity.portNumber, target->trp,
	...
	case PORT_DATA_SET_NP:
        ...
        memset(pdsnp->PortName, 0, sizeof(pdsnp->PortName));
		strncpy((char *)pdsnp->PortName, target->name, sizeof(pdsnp->PortName));
			       ...
			       struct ptp_message *req)
{
	...
	if (respond && !port_management_get_response(target, ingress, id, req))
		pr_err("PortDev[%u] failed to send management set response", portnum(target));
	return respond ? 1 : 0;
}
...
static void port_nrate_calculate(struct port *p, tmv_t t3, tmv_t t4, tmv_t c)
{
	...
	if (tmv_eq(t4, n->ingress1)) {
		pr_warning("PortDev[%u] bad timestamps in nrate calculation", portnum(p));
		return;
	}
	n->ratio =
				 ...
				 enum port_state next, enum fsm_event event)
{
	if (event == EV_FAULT_DETECTED) {
		pr_notice("PortDev[%u] %s to %s on %s (%s)", portnum(p),
			  ps_str[p->state], ps_str[next], ev_str[event],
			  ft_str(last_fault_type(p)));
	} else {
		pr_notice("PortDev[%u] %s to %s on %s", portnum(p),
			  ps_str[p->state], ps_str[next], ev_str[event]);
	}
}
...
static void port_slave_priority_warning(struct port *p)
{
	...
	pr_warning("PortDev[%u] master state recommended in slave only mode", n);
	pr_warning("PortDev[%u] defaultDS.priority1 probably misconfigured", n);
}
...
static int port_pdelay_request(struct port *p)
{
	...
	cnt = transport_peer(p->portIdentity.portNumber, p->trp, &p->fda, 1, msg, pdulen, &msg->hwts);
	if (cnt <= 0) {
		pr_err("PortDev[%u] send peer delay request failed", portnum(p));
		goto out;
	}
	if (msg_sots_missing(msg)) {
		pr_err("PortDev[%u] missing timestamp on transmitted peer delay request", portnum(p));
		goto out;
	}
...
static int port_delay_request(struct port *p)
{
	...
	cnt = transport_send(p->portIdentity.portNumber, p->trp, &p->fda, 1, msg, pdulen, &msg->hwts);
	if (cnt <= 0) {
		pr_err("PortDev[%u] send delay request failed", portnum(p));
		goto out;
	}
	if (msg_sots_missing(msg)) {
		pr_err("PortDev[%u] missing timestamp on transmitted delay request", portnum(p));
		goto out;
	}
...
static int port_tx_announce(struct port *p)
{
	...
	cnt = transport_send(p->portIdentity.portNumber, p->trp, &p->fda, 0, msg, pdulen, &msg->hwts);
	if (cnt <= 0) {
		pr_err("PortDev[%u] send announce failed", portnum(p));
		err = -1;
	}
out:
	msg_put(msg);
...
static int port_tx_announce_transparent(struct port *p, struct ptp_message *m)
{
	struct parent_ds *dad = clock_parent_ds(p->clock);
	struct ptp_message *msg;
	int cnt, err = 0, pdulen;
	...
	msg->header.sourcePortIdentity = m->header.sourcePortIdentity;
	msg->header.sequenceId         = m->header.sequenceId;
	msg->header.control            = m->header.control;
	msg->header.logMessageInterval = m->header.logMessageInterval;

	msg->header.flagField[1] = m->header.flagField[1];

	msg->announce.currentUtcOffset        = m->announce.currentUtcOffset;
	msg->announce.grandmasterPriority1    = m->announce.grandmasterPriority1;
	msg->announce.grandmasterClockQuality = m->announce.grandmasterClockQuality;
	msg->announce.grandmasterPriority2    = m->announce.grandmasterPriority2;
	msg->announce.grandmasterIdentity     = m->announce.grandmasterIdentity;
	msg->announce.stepsRemoved            = m->announce.stepsRemoved + 1;
	msg->announce.timeSource              = m->announce.timeSource;
    ...
    //pr_info("TRANSPARENT: ANNOUNCE FORWARDED");
out:
	msg_put(msg);
	return err;
}
...
static int port_tx_sync(struct port *p)
{
	...
	cnt = transport_send(p->portIdentity.portNumber, p->trp, &p->fda, event, msg, pdulen, &msg->hwts);
	if (cnt <= 0) {
		pr_err("PortDev[%u] send sync failed", portnum(p));
		err = -1;
		goto out;
	}
	if (p->timestamping == TS_ONESTEP) {
		...
		pr_err("PortDev[%u] missing timestamp on transmitted sync", portnum(p));
		err = -1;
		goto out;
	}
	...
	cnt = transport_send(p->portIdentity.portNumber, p->trp, &p->fda, 0, fup, pdulen, &fup->hwts);
	if (cnt <= 0) {
		pr_err("PortDev[%u] send follow up failed", portnum(p));
		err = -1;
	}
out:
	msg_put(msg);
...
static void port_disable(struct port *p)
{
	...
	transport_close(p->portIdentity.portNumber, p->trp, &p->fda);
...
static int port_initialize(struct port *p)
{
	...
	for (i = 0; i < N_TIMER_FDS; i++) {
		...
		if (fd[i] < 0) {
			pr_err("PortDev[%u] timerfd_create: %s", portnum(p), strerror(errno));
			goto no_timers;
		}
	}
	if (transport_open(p->portIdentity.portNumber, p->trp, p->name, &p->fda, p->timestamping))
		goto no_tropen;
 ...
 */
static int process_announce(struct port **allports, int nports, struct port *p, struct ptp_message *m)
{
	int i, result = 0;
    struct port *port;
	...
	case PS_SLAVE:
        ...
        if (Query_Clock_Mode(p->clock) == ClockMode_TC)         //is transparent
        {
            for (i = 0; i < nports; i++)
            {
                port = allports[i];
                ...
                if (port != p)
                {
                    port_tx_announce_transparent(port, m);            
                }
            }
        }
		break;
...
static int process_delay_req(struct port *p, struct ptp_message *m)
{
	...
	if (p->delayMechanism == DM_P2P) {
		pr_warning("PortDev[%u] delay request on P2P port", portnum(p));
		return 0;
	}
	...
	cnt = transport_send(p->portIdentity.portNumber, p->trp, &p->fda, 0, msg, pdulen, NULL);
	if (cnt <= 0) {
		pr_err("PortDev[%u] send delay response failed", portnum(p));
		err = -1;
	}
out:
	msg_put(msg);
...
static void process_delay_resp(struct port *p, struct ptp_message *m)
{
	...
	if (p->logMinDelayReqInterval != rsp->hdr.logMessageInterval) {
		...
		pr_notice("PortDev[%u] minimum delay request interval 2^%d",
			portnum(p), p->logMinDelayReqInterval);
	}
}

static void process_follow_up(struct port **allports, int nports, struct port *p, struct ptp_message *m)
{   
    struct port *port;
    int i;
    tmv_t pdelay;
	enum syfu_event event;
    //struct timespec sync_ingress;
	struct PortIdentity master;
    ...
    /**
     * added mechanism to send to multiple ports
     * added transparentMode flag check
     */
    if (Query_Clock_Mode(p->clock) == ClockMode_TC)
    {
        struct ptp_message * msg_copy = msg_allocate();
        int msglen = sizeof(struct follow_up_msg);         //m->header.messageLength;
        for (i = 0; i < nports; i++)
        {
            port = allports[i];
            if (port != p)
            {
                if (Port_QueryState(port) != PS_FAULTY)
                {
                    // forward_follow_up alters message contents, so we need a fresh copy each time
                    memcpy(msg_copy, m, sizeof(struct ptp_message));
                    pdelay = p->peer_delay;
                    forward_follow_up(port, msg_copy, msglen, pdelay);
                }                        
            }
        }
        msg_put(msg_copy);
...
static int process_pdelay_req(struct port *p, struct ptp_message *m)
{
	...
	if (p->delayMechanism == DM_E2E) {
		pr_warning("PortDev[%u] pdelay_req on E2E port", portnum(p));
		return 0;
	}
	if (p->delayMechanism == DM_AUTO) {
		pr_info("PortDev[%u] peer detected, switch to P2P", portnum(p));
		p->delayMechanism = DM_P2P;
		port_set_delay_tmo(p);
	}
	if (p->peer_portid_valid) {
		if (!pid_eq(&p->peer_portid, &m->header.sourcePortIdentity)) {
			pr_err("PortDev[%u] received pdelay_req msg with "
		...
		pr_debug("PortDev[%u] peer port id set to %s", portnum(p),
	...
	cnt = transport_peer(p->portIdentity.portNumber, p->trp, &p->fda, 1, rsp, rsp_len, &rsp->hwts);
	if (cnt <= 0) {
		pr_err("PortDev[%u] send peer delay response failed", portnum(p));
		goto out;
	}
	if (msg_sots_missing(rsp)) {
		pr_err("PortDev[%u] missing timestamp on transmitted peer delay response", portnum(p));
		goto out;
	}
	...
	cnt = transport_peer(p->portIdentity.portNumber, p->trp, &p->fda, 0, fup, fup_len, &rsp->hwts);
	if (cnt <= 0) {
		pr_err("PortDev[%u] send pdelay_resp_fup failed", portnum(p));
		goto out;
	}
	err = 0;
out:
...
static void port_peer_delay(struct port *p)
{
    ...
    pr_debug("PortDev[%u] t1 = %lld t2 = %lld t3 = %lld t4 = %lld c1 = %lld c2 = %lld", 
                    portnum(p),
                    tmv_to_nanoseconds(t1),
                    tmv_to_nanoseconds(t2),
                    tmv_to_nanoseconds(t3),
                    tmv_to_nanoseconds(t4),
                    tmv_to_nanoseconds(c1),
                    tmv_to_nanoseconds(c2)); 
calc:
...
static int process_pdelay_resp(struct port *p, struct ptp_message *m)
{
	if (p->peer_delay_resp) {
		if (!source_pid_eq(p->peer_delay_resp, m)) {
			pr_err("PortDev[%u] multiple peer responses", portnum(p));
			if (!p->multiple_pdr_detected) {
	...
	if (!p->peer_delay_req) {
		pr_err("PortDev[%u] rogue peer delay response", portnum(p));
		return -1;
	}
	if (p->peer_portid_valid) {
		if (!pid_eq(&p->peer_portid, &m->header.sourcePortIdentity)) {
			pr_err("PortDev[%u] received pdelay_resp msg with unexpected peer port id %s",
				portnum(p),
				pid2str(&m->header.sourcePortIdentity));
			p->peer_portid_valid = 0;
			port_capable(p);
...
static void process_sync(struct port **allports, int nports, struct port *p, struct ptp_message *m)
{
	enum syfu_event event;
	struct PortIdentity master;
    struct ptp_message *msg_copy;
    ...
    /*
        For Transparet Clock, send SYNC to all egress ports
    */
    if (Query_Clock_Mode(p->clock) == ClockMode_TC)
    {
        port_set_singress(p->clock, timespec_to_tmv(m->hwts.ts));

        struct port *port;
        int i, msglen;

        // port_forward alters message contents, so we need a fresh copy each time
        msg_copy = msg_allocate();
        msglen = sizeof(struct sync_msg);
        ...
        for (i = 0; i < nports; i++)
        {
            ...
            if (p != port)
            {
                ...
                {
                    memcpy(msg_copy, m, sizeof(struct ptp_message));
                    pr_debug("PortDev[%u] TRANSPARENT: forwarding sync", portnum(p));
                    port_forward(port, msg_copy, msglen);
                }
            }
        }
        msg_put(msg_copy);
	...
	if (p->syfu == SF_HAVE_FUP &&
	    ...
	    p->last_syncfup->header.sequenceId == m->header.sequenceId) {
            pr_debug("PortDev[%u] reached sync stage: MATCH", portnum(p));
		event = SYNC_MATCH;
	} else {
        pr_debug("PortDev[%u] reached sync stage: MISMATCH", portnum(p));
		event = SYNC_MISMATCH;
	}
	port_syfufsm(p, event, m);
}
...
enum fsm_event port_event(struct port **allports, int nports, struct port *p, int fd_index)
{
	...
	case FD_SYNC_RX_TIMER:
		pr_debug("PortDev[%u] %s timeout", portnum(p),
	...
	case FD_DELAY_TIMER:
		pr_debug("PortDev[%u] delay timeout", portnum(p));
		port_set_delay_tmo(p);
		return port_delay_request(p) ? EV_FAULT_DETECTED : EV_NONE;

	case FD_QUALIFICATION_TIMER:
		pr_debug("PortDev[%u] qualification timeout", portnum(p));
		return EV_QUALIFICATION_TIMEOUT_EXPIRES;

	case FD_MANNO_TIMER:
		pr_debug("PortDev[%u] master tx announce timeout", portnum(p));
        if (Query_Clock_Mode(p->clock) != ClockMode_TC)        //not transparent
        {
            pr_debug("PortDev[%u] BOUNDARY: SENDING MASTER ANNOUNCE", portnum(p));
	...
	case FD_SYNC_TX_TIMER:
		pr_debug("PortDev[%u] master sync timeout", portnum(p));
        if (Query_Clock_Mode(p->clock) != ClockMode_TC)        //not transparent
        {   
            pr_debug("PortDev[%u] BOUNDARY: SENDING SYNC", portnum(p));
		    port_set_sync_tx_tmo(p);
	...
	cnt = transport_recv(p->portIdentity.portNumber, p->trp, fd, msg, sizeof(msg->data), &msg->hwts);
	if (cnt <= 0) {
		pr_err("PortDev[%u] recv message failed", portnum(p));
		msg_put(msg);
		return EV_FAULT_DETECTED;
	}
	...
	if (err) {
		switch (err) {
		case -EBADMSG:
			pr_err("PortDev[%u] bad message", portnum(p));
			break;
		case -ETIME:
			pr_err("PortDev[%u] received %s without timestamp", portnum(p), msg_type_string(msg_type(msg)));
			break;
		case -EPROTO:
			pr_debug("PortDev[%u] ignoring message", portnum(p));
		}
		msg_put(msg);
		return EV_NONE;
	}
	...
	switch (msg_type(msg)) {
	case SYNC:
		process_sync(allports, nports, p, msg);
		break;
	case DELAY_REQ:
	...
	case PDELAY_RESP:
		...
		break;
	case FOLLOW_UP:
		process_follow_up(allports, nports, p, msg);
		break;
	case DELAY_RESP:
	...
	case PDELAY_RESP_FOLLOW_UP:
		...
		break;
	case ANNOUNCE:
		if (process_announce(allports, nports, p, msg))
        {
...
{   
    ...
    {   
    	...
    	cnt = transport_send(p->portIdentity.portNumber, p->trp, &p->fda, 1, msg, msglen, &msg->hwts);      //1 is different type of event, allowing ts

        if (msg_sots_missing(msg))
        {
            pr_err("PortDev[%u] missing timestamp on forwarded sync", portnum(p));
        }
        port_set_segress(p->clock, timespec_to_tmv(msg->hwts.ts), portnum(p));
    }
    else
    {
    	cnt = transport_send(p->portIdentity.portNumber, p->trp, &p->fda, 0, msg, msglen, &msg->hwts);
    }    
	return cnt <= 0 ? -1 : 0;
}

/*
 * added follow-up forward
 * this takes a follow-up message and forwards it on the specified ports
 * while adding the correction field and the correct preciseOriginTimestamp
 * from original sync
 */
int forward_follow_up(struct port *p, struct ptp_message *msg, int msglen, tmv_t peer_delay)
{
    timestamp_to_Timestamp(&msg->ts.pdu, &msg->follow_up.preciseOriginTimestamp);
    tmv_t egress = port_get_segress(p->clock, portnum(p));
    tmv_t ingress = port_get_singress(p->clock);
    
    if (p->pod.follow_up_info)
    {
		msglen += follow_up_info_append(p, msg);
    }    
    msg->header.correction = (Integer64) calc_correction(ingress, egress, peer_delay, msg->header.correction);
...
/**
    @brief  Open a PTP or UDS port

    @param  phc_index
    @param  timestamping
    @param  number
    @param  interface
    @param  clock           Pointer to our local clock

    @return Pointer to the port definition
*/
                       ...
                       struct clock *clock)
{
	...
	if (interface->transport == TRANS_UDS)
		...
		pr_warning("PortDev[%u] get_ts_info not supported", number);
	else if (phc_index >= 0 && phc_index != interface->ts_info.phc_index) {
		pr_err("PortDev[%u] PHC device mismatch", number);
		pr_err("PortDev[%u] /dev/ptp%d requested, but /dev/ptp%d attached",
	...
	if (!p->delay_filter) {
		pr_err("PortDev[%u] Failed to create delay filter", number);
		transport_destroy(p->trp);
		free(p);
		return NULL;
	}
