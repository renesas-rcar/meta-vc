...
static int uds_close(int PortNumber, struct transport *t, struct fdarray *fda)
{
	close(fda->fd[FD_GENERAL]);
	return 0;
}

static int uds_open(int PortNumber, struct transport *t, const char *name, struct fdarray *fda,
		    enum timestamp_type tt)
{
	...
	if (fd < 0) {
		pr_err("PortDev[%u] uds: failed to create socket: %m", PortNumber);
		return -1;
	}
	...
	if (err < 0) {
		pr_err("PortDev[%u] uds bind failed: %m", PortNumber);
		close(fd);
		return -1;
	}
...
static int uds_recv(int PortNumber, struct transport *t, int fd, void *buf, int buflen,
		    struct hw_timestamp *hwts)
{
	...
	if (cnt <= 0) {
		pr_err("PortDev[%u] uds recvfrom failed: %m", PortNumber);
	}
	return cnt;
}

static int uds_send(int PortNumber, struct transport *t, struct fdarray *fda, int event,
		    int peer, void *buf, int buflen, struct hw_timestamp *hwts)
{
	...
	if (cnt <= 0) {
		pr_err("PortDev[%u] uds sendto failed: %m", PortNumber);
	}
	return cnt;
}
